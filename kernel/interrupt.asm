...
; Declare global functions defined in this file
global isr0
global isr1
global isr2
global isr3
...
; Import external functions from C code
extern isr_handler
extern irq_handler

; Common ISR stub. Saves CPU state, sets up segments, calls C exception handler, restores CPU state.
isr_common_stub:
    pusha                    ; Pushes edi, esi, ebp, esp, ebx, edx, ecx, eax
    mov ax, ds
    push eax                 ; Save data segment descriptor
    mov ax, 0x10             ; Kernel Data Segment descriptor (0x10)
    mov ds, ax
    mov es, ax
    mov fs, ax
    mov gs, ax
...
; Common IRQ stub. Saves CPU state, sets up segments, calls C IRQ handler, restores CPU state.
irq_common_stub:
    pusha                    ; Pushes edi, esi, ebp, esp, ebx, edx, ecx, eax
    mov ax, ds
    push eax                 ; Save data segment descriptor
...
isr8:
    cli
    push byte 8              ; Exception number (error code is already pushed by CPU)
    jmp isr_common_stub
